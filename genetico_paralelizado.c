...
/*Programa en C que realiza la exploración de un laberinto haciendo uso de un algoritmo genetico. El rango de valores de x comprende entre
0 y 3.*/
...
int main(){
	...
	printf("\nEl valor maximo encontrado es: %d \n", (int)sqrt(m_max));
...
void reproduccion(int np1, int mb1){
	...
	do{
		...
		//Se utiliza una variable temporal para almacenar los bits a ser intercambiados entre ambas cadenas de bits.
		if((k == 0) && (l != 0)){
			for(j=0; j<l; j++){
				temp = npop[i][j];
				npop[i][j] = npop[i+1][j];
